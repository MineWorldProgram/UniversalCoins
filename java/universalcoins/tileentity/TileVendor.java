...
public class TileVendor extends TileProtected implements IInventory, ISidedInventory {
	...
	private void activateSellButton() {
		if (!inventory.get(itemSellSlot).isEmpty() && inventory.get(itemTradeSlot).getItem() == inventory.get(itemSellSlot).getItem()
	...
	private void activateRetrieveButtons() {
		...
		if (coinSum > 0) {
			ironCoinBtnActive = inventory.get(itemOutputSlot).isEmpty()
					|| (inventory.get(itemOutputSlot).getItem() == UniversalCoins.proxy.iron_coin
	...
	public int retrieveCoins(int coinField, int buttonClickedID, boolean shiftPressed) {
		...
		if (coinField < UniversalCoins.coinValues[absoluteButton]
				|| (!inventory.get(itemCoinOutputSlot).isEmpty() && inventory.get(itemCoinOutputSlot).getItem() != itemOnButton)
				|| (!inventory.get(itemCoinOutputSlot).isEmpty() && inventory.get(itemCoinOutputSlot).getCount() == 64)) {
			return coinField;
		}
		if (shiftPressed) {
			if (inventory.get(itemCoinOutputSlot).isEmpty()) {
				...
				int amount = Math.min(coinSum / UniversalCoins.coinValues[absoluteButton],
						inventory.get(itemCoinOutputSlot).getMaxStackSize() - inventory.get(itemCoinOutputSlot).getCount());
				inventory.get(itemCoinOutputSlot).setCount(inventory.get(itemCoinOutputSlot).getCount() + amount);
				coinField -= UniversalCoins.coinValues[absoluteButton] * amount;
			}
		} else {
			...
			if (inventory.get(itemCoinOutputSlot).isEmpty()) {
				...
				inventory.get(itemCoinOutputSlot).setCount(inventory.get(itemCoinOutputSlot).getCount() + 1);
			}
		}
		return coinField;
	}
	...
	private void activateUserRetrieveButtons() {
		...
		if (userCoinSum > 0) {
			uironCoinBtnActive = inventory.get(itemCoinOutputSlot).isEmpty()
					|| (inventory.get(itemCoinOutputSlot).getItem() == UniversalCoins.proxy.iron_coin
	...
	public void onBuyPressed(int amount) {
		...
		if (!inventory.get(itemOutputSlot).isEmpty()
				&& inventory.get(itemOutputSlot).getCount() + totalSale > inventory.get(itemTradeSlot).getMaxStackSize()) {
			buyButtonActive = false;
			return;
		}
		if (infiniteMode) {
			if (inventory.get(itemOutputSlot).isEmpty()) {
				...
				inventory.get(itemOutputSlot).setCount(inventory.get(itemOutputSlot).getCount() + totalSale);
				if (useCard && !inventory.get(itemUserCardSlot).isEmpty()) {
					debitUserAccount(itemPrice * amount);
				} else {
					userCoinSum -= itemPrice * amount;
			...
			for (int i = itemStorageSlot1; i <= itemStorageSlot9; i++) {
				if (!inventory.get(i).isEmpty() && inventory.get(i).getItem() == inventory.get(itemTradeSlot).getItem()
					...
					inventory.get(itemOutputSlot).setCount(inventory.get(itemOutputSlot).getCount() + thisSale);
					inventory.get(i).setCount(inventory.get(i).getCount() - thisSale);
					totalSale -= thisSale;
	...
	public void onSellPressed(int amount) {
		...
		if (infiniteMode) {
			if (!inventory.get(itemUserCardSlot).isEmpty()
					&& inventory.get(itemUserCardSlot).getItem() == UniversalCoins.proxy.ender_card
			...
			inventory.get(itemSellSlot).setCount(inventory.get(itemSellSlot).getCount() - amount);
		...
		for (int i = itemStorageSlot1; i <= itemStorageSlot9; i++) {
			if (!inventory.get(itemSellSlot).isEmpty()) {
				...
				if (!inventory.get(i).isEmpty() && inventory.get(i).getItem() == inventory.get(itemTradeSlot).getItem()
					...
					inventory.get(i).setCount(inventory.get(i).getCount() + thisSale);
					inventory.get(itemSellSlot).setCount(inventory.get(itemSellSlot).getCount() - thisSale);
					amount -= thisSale;
				...
				if (!inventory.get(itemUserCardSlot).isEmpty()
						...
						&& getUserAccountBalance() != -1 && getUserAccountBalance()
								+ (itemPrice * thisSale / inventory.get(itemTradeSlot).getCount()) < Integer.MAX_VALUE) {
	...
	public long getUserAccountBalance() {
		if (!inventory.get(itemUserCardSlot).isEmpty() && inventory.get(itemUserCardSlot).hasTagCompound() && !world.isRemote) {
